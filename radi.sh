...
function showDetails(){
    ...
    echo -e "\n 8 - Install hotspot cron: This option is intended to create a method to access the RADD when it does not establish other WLAN connectivity. This option asks for user input for SSID and passphrase (special characters should be avoided). and then builds a script in the working directory and a cron job called /etc/cron.d/hotspot This can be useful for set-up and testing, but should be disabled (move or remove the cron file or script) prior to sending a RADD into the wild. Stop the hotspot with: sudo nmcli device disconnect wlan0"
